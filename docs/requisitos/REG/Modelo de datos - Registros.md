...
| entidad           | Entidad federativa, **solo dentro de México**: fuera se queda vacía porque nunca se pregunta (`CU-REG-001` A2). Se guarda el **código de tres letras de ISO 3166-2:MX** (`YUC`), no el nombre, por lo mismo que el país guarda `MX` — «Ciudad de México», «CDMX» y «Distrito Federal» son la misma entidad y un catálogo libre las cuenta como tres. Catálogo en `filey/apps/registros/estados_mx.py`, con Yucatán por omisión. **En el código el campo se llama `entidad` y no `estado`**: ese nombre ya lo ocupa el estado de la cuenta, aquí abajo. Su etiqueta en pantalla es «Estado». |
| ciudad            | Ciudad de residencia, **opcional** y solo dentro de México. Texto libre a propósito: no hay catálogo de municipios que valga la pena mantener para esto, y exigirla dejaría fuera a quien escribe desde una localidad cuyo nombre oficial no conoce. Lo que sitúa a la persona es la entidad. |
| estado            | `activa` / `inactiva`. **Es el estado de la cuenta**, no una entidad federativa; se rotula así en el admin desde que existe `entidad`.                          |
