...
estructura navegable de dos áreas: el **Portal del Usuario** (editorial/entidad
...
- Los casos de uso del actor **Sistema** (procesos automáticos y temporizados) **no
  tienen ventana**: se manifiestan como correos o como cambios de estado que el usuario
...
- **Acceso condicionado:** una vista marcada así solo se habilita si se cumple su
  precondición (p. ej. *aplicación aceptada*, RN-16).
...
flowchart LR
    ...
    ROL -->|Usuario| U1[U1 · Aplicación a expositor]
    U1 -->|aplicación aceptada| U2[U2 · Mapa del showfloor]
    U2 --> U3[U3 · Carrito]
    ...
    A1 --> A2[A2 · Detalle de aplicación]
    A1 -.-> A3[A3 · Reservas]
    ...
    class U1,U2,U3,U4,U5,U6 usuario;
...
## 4. Portal del Usuario

> Acceso global: todas las vistas U2–U6 requieren **aplicación aceptada** (RN-16). Hasta
> entonces el usuario solo ve `U1`.

### U1 · Aplicación a expositor
...
- **Contenido:** mapa con stands en estado **Disponible / No disponible** (el usuario no
...
### A2 · Detalle de aplicación
- **Objetivo:** revisar y resolver una aplicación.
...
- **CU involucrados:** CU-STD-005 (revisar detalle), CU-STD-006 (aceptar o rechazar), CU-STD-007
  (solicitar cambios). Dispara CU-STD-008 (notificación al usuario, Sistema).
...
- **Contenido:** lista de expositores con **aplicación aceptada**.
...
- **Contenido:** mapa con **quién reservó cada stand** y el **saldo pendiente**; a
  diferencia del mapa del usuario, distingue todos los estados.
...
|----|---------|------------------|
| CU-STD-008 | Notificar el resultado de la aplicación | Correo · estado en U1 |
...
| CU-STD-026 | Notificar al usuario posible cancelación | Correo · banner en U5 |
...
| Vista | CU |
|-------|----|
| U1 · Aplicación a expositor | CU-STD-001, CU-STD-002, CU-STD-003 |
...
| A2 · Detalle de aplicación | CU-STD-005, CU-STD-006, CU-STD-007 |
