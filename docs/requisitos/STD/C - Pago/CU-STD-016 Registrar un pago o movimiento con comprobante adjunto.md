...
El usuario registra un abono realizado para su reserva, ingresando el monto, el método utilizado y subiendo el comprobante oficial. El movimiento queda en estado pendiente hasta ser validado por el administrador.
...
- El usuario tiene una reserva registrada en estado `Por confirmar` o `Confirmada`.
- La reserva tiene un saldo pendiente mayor a cero.

## Disparador

El usuario realiza un pago bancario y decide registrar el comprobante en la plataforma para que se acredite a su reserva.

## Flujo principal

1. El usuario accede a la sección de pagos de su reserva y selecciona la opción de "Registrar abono".
...
3. El usuario completa los datos y adjunta el comprobante bancario.
...
6. El sistema crea un registro en la entidad `Movimiento` asociado a la reserva, con origen `usuario` y estado `pendiente_validacion`.
7. El sistema guarda el comprobante en la entidad `Documento` vinculándolo al movimiento.
8. El sistema muestra un mensaje de confirmación al usuario, indicando que su pago está en revisión por el administrador.
...
3. El usuario corrige la información y reintenta el envío (regresa al paso 4).
...
3. El usuario ajusta el monto y reintenta el envío (regresa al paso 4).
...
- **Fallo:** No se registra el movimiento si el usuario abandona el formulario o no adjunta el comprobante.
