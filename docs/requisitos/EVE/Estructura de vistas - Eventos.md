...
- Los casos de uso del actor **Sistema** (correos y procesos automáticos) **no tienen
  ...
  pantallas (ver §6).
- La sección **C · Programación** (CU-EVE-015 a 023) corresponde a **otro integrante del
...
- **CU involucrados:** CU-EVE-002 (enviar propuesta), CU-EVE-003 (múltiples propuestas sin
  recapturar contacto).
...
- **Contenido:** lista con folio, tipo, título y estado (`pendiente` /
  ...
  vistas P4 y P5.
- **CU involucrados:** CU-EVE-036 (consultar mis propuestas y su estado).
- **Entidades:** Propuesta.
...
- **CU involucrados:** CU-EVE-036 (detalle), CU-EVE-004 (editar y reenviar tras solicitud
  de cambios).
...
- **CU involucrados:** CU-EVE-024 (responder notificación de horario).
- **Reflejo de Sistema:** CU-EVE-023 (notificación de horario) llega como correo y aparece
  aquí.
...
- **CU involucrados:** CU-EVE-028 (descargar constancia).
...
- **Contenido:** lista filtrable por tipo, estado y categoría; **contador en tiempo real**
  ...
  alcanzado*).
- **CU involucrados:** CU-EVE-007 (lista), CU-EVE-012 (contador).
...
- **Contenido:** datos del proponente, datos de la actividad y adjuntos; concentra las
  ...
  - **Dictaminar**: Aceptar (2ª pantalla, clasifica `literaria`/`academica`, crea
    ...
    inmediato) o Rechazar (motivo obligatorio) — CU-EVE-009. Incluye **re-dictamen** con
    doble verificación.
  - **Marcar recepción del ejemplar físico** (presentaciones de libro/revista) —
    CU-EVE-014.
  - **Ver bitácora** de la actividad asociada — CU-EVE-035 (segunda ruta de acceso).
- **CU involucrados:** CU-EVE-008 (detalle), CU-EVE-009, CU-EVE-014.
...
- **Contenido:** alcance (completo o filtrado por categoría/sección/tipo/día/sala) y
  ...
  la **ficha PDF individual** (CU-EVE-031).
- **CU involucrados:** CU-EVE-030 (exportar), CU-EVE-031 (ficha PDF).
...
- **Contenido:** acción **Cerrar definitivamente** (motivo obligatorio + doble
  ...
  programa cerrado.
- **CU involucrados:** CU-EVE-027 (cerrar/reabrir), CU-EVE-026 (cambio excepcional).
...
- **Contenido:** listado de actividades de la versión publicada (título, tipo, fecha, sala,
  ...
  actividades sin horario fijo. Acción de **ficha PDF** por actividad (CU-EVE-031).
- **CU involucrados:** CU-EVE-032, CU-EVE-031.
...
- **Servicio de correo:** sin definir; afecta a CU-EVE-009 (solicitud de cambios),
  CU-EVE-013 (lote) y CU-EVE-023 (horario).
...
- **Ficha PDF (CU-EVE-031):** definir si se ofrece tanto en la cartelera pública (V1) como
