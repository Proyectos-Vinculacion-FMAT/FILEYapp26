...
> [!note] Homologación de numeración de CU (2026-06-29)
> Este documento se escribió antes de homologar la numeración final de `CU-EVT Índice.md`.
> Las referencias a **CU-EVT-001 a CU-EVT-012** ya están actualizadas a la numeración vigente.
> Las referencias a números **mayores a 012** (p. ej. CU-EVT-013 a CU-EVT-034) corresponden a
> casos de uso de la visión original que **todavía no se redactaron como archivo** —
> Programación (delegada por completo a `PRG`), Confirmación/Cierre y Publicación más allá de
> la ficha PDF y la constancia. Se conservan aquí como referencia de alcance futuro, no como
> CUs vigentes.
...
| categoria                   | `literaria_uady` / `literaria_externa` / `academica_uady` / `academica_externa`. El sufijo `_uady` / `_externo` lo deriva el sistema del `tipo_participante` del proponente al enviar; el prefijo `literaria` / `academica` lo asigna el administrador durante el dictamen (CU-EVT-009). |
...
| resultado_notificado        | Booleano — si el resultado vigente (`aceptada` / `rechazada`) ya fue comunicado al proponente en un lote (CU-EVT-010). Se restablece a `false` cuando el administrador cambia el dictamen (re-dictamen).                                                                                 |
...
> [!note]
> Estos campos se usan en la vista de detalle de la actividad pública (CU-EVT-032). El estado `en_negociacion` que los acompañaba pertenece a v2 (CU-EVT-010 pospuesto).
...
| programa_archivado              | Booleano — si el programa fue cerrado definitivamente (CU-EVT-026).       |
| fecha_archivado                 | Fecha y hora en que se ejecutó el cierre definitivo.                      |
| archivado_por                   | FK → Persona (admin que ejecutó el cierre).                               |
| motivo_archivado                | Texto — motivo registrado al archivar (obligatorio según CU-EVT-026).     |
...
| ProgramacionActividad / SesionActividad | CU-EVT-014, CU-EVT-015, CU-EVT-016, CU-EVT-017, CU-EVT-019, CU-EVT-020, CU-EVT-022             |
| ConfirmacionProponente                  | CU-EVT-022, CU-EVT-023                                                                         |
| NotificacionLote                        | CU-EVT-010, CU-EVT-022                                                                         |
| ParametrosConvocatoria                  | CU-EVT-001, CU-EVT-026                                                                         |
| BitacoraEVE                             | CU-EVT-025, CU-EVT-026, CU-EVT-034                                                             |
