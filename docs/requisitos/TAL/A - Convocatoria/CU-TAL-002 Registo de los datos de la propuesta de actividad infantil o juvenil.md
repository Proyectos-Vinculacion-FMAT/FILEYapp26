...
> Integra el llenado del formulario y el envío de la propuesta (absorbe CU-TAL-003).
> Equivale a CU-EVT-002 + CU-EVT-002.1 pero adaptado a la convocatoria infantil/juvenil
> de Elvira: sin dictamen por propuesta en el sistema, sin adjuntos de archivo y con campos
> propios de este tipo de actividad.
...
Módulo de Talleres (TAL) — formulario de registro y envío de propuesta infantil/juvenil. El tallerista debe estar autenticado (CU-REG-002). Cubre la captura, la validación local y el envío de la propuesta. No cubre la edición posterior (CU-TAL-005) ni el registro de visitas escolares (CU-TAL-007). A diferencia de CU-EVT-002, no existe paso de categorización ni carga de archivos adjuntos; tampoco hay dictamen por propuesta.
...
- Se crea un registro `PropuestaTaller` en estado `registrada`, con folio y fecha de envío asignados por el sistema.
- El tallerista recibe por correo la confirmación de recepción con su número de folio.
- La selección de propuestas la realiza la coordinación (Elvira) de forma manual fuera del sistema; la confirmación de participación se comunica por correo en diciembre.
...
12. El sistema crea el registro `PropuestaTaller` en estado `registrada` con folio y fecha de envío.
...
- `PropuestaTaller` en estado `registrada` con folio y fecha de envío.
